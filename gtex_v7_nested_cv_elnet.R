...
get_maf_filtered_genotype <- function(genotype_file_name,  maf, samples) {
  ...
  gt_df <- gt_df[,which((effect_allele_freqs >= as.double(maf)) | (effect_allele_freqs <= (1 - as.double(maf))))]
...
get_cis_genotype <- function(gt_df, snp_annot, coords, cis_window, LA_Inter) {### Last parameter added by Layan Nahlawi
  ...
  cis_gt_col_names<-colnames(cis_gt)
  ###
  # Addition by Layan Nahlawi
  print("Started getting the corresponding Local ancestry columns and interaction columns")
  if (length(intersect(paste(snp_info$varID, "_LA",sep=""), colnames(LA_Inter)))!=0){
    cis_gt <- cbind(cis_gt,LA_Inter[, intersect(paste(snp_info$varID, "_LA",sep=""), colnames(LA_Inter))])
    colnames(cis_gt)<- c(cis_gt_col_names,intersect(paste(snp_info$varID, "_LA",sep=""), colnames(LA_Inter)))
  }
  cis_gt_col_names<-colnames(cis_gt)
   if (length(intersect(paste(snp_info$varID, "_INT",sep=""), colnames(LA_Inter)))!=0){
     cis_gt <- cbind(cis_gt,LA_Inter[, intersect(paste(snp_info$varID, "_INT",sep=""), colnames(LA_Inter))])
     colnames(cis_gt)<- c(cis_gt_col_names,intersect(paste(snp_info$varID, "_INT",sep=""), colnames(LA_Inter)))
   }
  print("Done getting the corresponding Local ancestry columns and interaction columns")
  ###
...
###
#function added by Layan Nahlawi
#This function reads the file containing the candidate predictors into a data frame. 
#The candidate predictors include the genotype data, the unique local ancestry information and 
# the interaction (multiplication) between the genotype and the non unique local ancestry data.
get_candidate_predictors_local_ancestry <- function(LA_Inter_file ){
  print("Started reading The candidate predictors file into a data frame.")
  if (file.exists(LA_Inter_file)){
 	  LA_Inter <- fread(LA_Inter_file, stringsAsFactors = F, header=T, fill=T)
    LA_Inter <- as.data.frame(LA_Inter)
    print("Done reading The candidate predictors file into a data frame")
    LA_Inter
  }else{
      stop(paste0("File: ",LA_Inter_file," does NOT exist."))
  }
}
###
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
                 covariates_file, LA_Inter_file, chrom, prefix, maf=0.01, n_folds=60, n_train_test_folds=5,
                 seed=NA, cis_window=1e6, alpha=0.5, null_testing=FALSE ) {### 6th parameter added by Layan Nahlawi
  ...
  #Addition by Layan Nahlawi
  print(LA_Inter_file)
  LA_Inter<-get_candidate_predictors_local_ancestry(LA_Inter_file)
  ###
  ...
  for (i in 1:n_genes) {
    ...
    # if(i==17)
    #   browser()
    cis_gt <- get_cis_genotype(gt_df, snp_annot, coords, cis_window,LA_Inter ) ### last term added by Layan Nahlawi
    if (all(is.na(cis_gt))) {
      # No snps within window for gene.
      print("No snps within window for gene.")
    ...
    if (ncol(cis_gt) >= 2) {
      ...
      if (length(fit) > 0) {
        ...
        if (fit$nzero[best_lam_ind] > 0) {
          print("just before filtering results")
          ...
          ###
          # Addition by Layan Nahlawi
          print("Started getting LA and Inter from the training results")
          # fwrite(as.data.frame(colnames(LA_Inter)),paste0("colNames_LA_Inter_", chrom ,".txt"))
          candidates_LA_Inter <- colnames(LA_Inter)
          weighted_snps_info_LA_inter <- as.data.frame(candidates_LA_Inter[(colnames(LA_Inter) %in% weighted_snps)])
          colnames(weighted_snps_info_LA_inter)<-"varID"
          
          if(!empty(weighted_snps_info_LA_inter)){
            weighted_snps_info_LA_inter$rsid_dbSNP150<-NA
            weighted_snps_info_LA_inter$ref_vcf<-NA
            weighted_snps_info_LA_inter$alt_vcf<-NA
            weighted_snps_info_LA_inter<-weighted_snps_info_LA_inter[c(2,1,3,4)]
            weighted_snps_info <- rbind(weighted_snps_info, weighted_snps_info_LA_inter, fill=T)
            fwrite(weighted_snps_info, paste0("../weights/weighted_snps_info",chrom,".txt"))
            print("Done getting LA and Inter from the training results")
          }else {
          print("no LA or INTER predictors were selected")
          }
          ###
...
print("test")
main(snp_annot_file, gene_annot_file, genotype_file, expression_file, covariates_file, LA_Inter_file, as.numeric(chrom), prefix, null_testing=FALSE, seed=01182019)
